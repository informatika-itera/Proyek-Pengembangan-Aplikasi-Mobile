...
import com.example.rosea.data.local.CartEntity
...
class CartRepositoryImpl(private val db: NoteDatabase) : CartRepository {
    private val queries = db.cartQueries

    override fun getCartItems(): Flow<List<CartItem>> {
        return queries.getCartItems()
            .asFlow()
            .mapToList(Dispatchers.Default)
            .map { entities: List<CartEntity> ->
                entities.map { entity ->
                    CartItem(
                        id = entity.id,
    ...
    override suspend fun updateCartItemQuantity(productId: Long, quantity: Long, addedAt: Long) {
        withContext(Dispatchers.Default) {
            queries.updateCartItemQuantity(quantity, addedAt, productId)
        }
    }

    override suspend fun deleteCartItem(productId: Long) {
        withContext(Dispatchers.Default) {
            queries.deleteCartItem(productId)
        }
    }

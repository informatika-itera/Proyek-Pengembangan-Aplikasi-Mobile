...
class GeminiService(
    ...
    suspend fun generateContent(prompt: String): Result<String> {
        return try {
            val response: GeminiResponseDto = client.post("${ApiConstants.GEMINI_BASE_URL}/models/${ApiConstants.GEMINI_MODEL}:generateContent?key=$apiKey") {
    ...
    suspend fun refineNote(rawNote: String): Result<String> {
        ...
        return generateContent(ApiConstants.Prompts.refineNote(rawNote))
    }

    suspend fun generateQuiz(noteContent: String): Result<String> {
        if (noteContent.isBlank()) return Result.failure(Exception("Konten materi kosong"))
        ...
        return generateContent(ApiConstants.Prompts.generateQuiz(noteContent))
    }
}

...
```bash
./gradlew :composeApp:generateCommonMainRailLogDatabaseInterface
```

> Nama task ini berasal dari konfigurasi di `composeApp/build.gradle.kts`:
> `sqldelight { databases { create("RailLogDatabase") { ... } } }`.
...
| `Cannot resolve symbol 'RailLogDatabase'`                 | Jalankan `./gradlew :composeApp:generateCommonMainRailLogDatabaseInterface`, lalu **Build → Rebuild Project**. |

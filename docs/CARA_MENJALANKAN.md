# 🚀 Cara Menjalankan neurodeck

Panduan lengkap untuk menjalankan template aplikasi **neurodeck** (Kotlin Multiplatform).
...
4. Edit `iosApp/iosApp/iOSApp.swift` agar memanggil `MainViewControllerKt.MainViewController()`
   ...
   `composeApp/src/iosMain/kotlin/com/example/neurodeck/MainViewController.kt`).
...
- **Debug DataStore**: file preferences disimpan di
  `/data/data/com.example.neurodeck/files/neurodeck.preferences_pb` (Android).

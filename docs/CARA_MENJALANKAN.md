# 🚀 Cara Menjalankan Hujjah

Panduan lengkap untuk menjalankan template aplikasi **Hujjah** (Kotlin Multiplatform).
...
4. Edit `iosApp/iosApp/iOSApp.swift` agar memanggil `MainViewControllerKt.MainViewController()`
   ...
   `composeApp/src/iosMain/kotlin/com/example/hujjah/MainViewController.kt`).
...
- **Debug DataStore**: file preferences disimpan di
  `/data/data/com.example.hujjah/files/hujjah.preferences_pb` (Android).
